...
async function V(s, t) {
  const a = s instanceof Uint8Array ? s : new Uint8Array(s), n = await crypto.subtle.importKey(
    ...
    !1,
    ["sign"]
  ), o = await crypto.subtle.sign("HMAC", n, new TextEncoder().encode(t));
...
async function ne(s) {
  ...
  try {
    ...
    const A = p instanceof Error ? p.message : String(p);
    return await $(n, { id: W(), targetId: t.id, shardId: r, path: i, sha256: "", size: 0, status: "failed", reason: A, at: c }), { status: "failed", reason: A };
  }
  if (m === null) {
    const p = "Document not found at source";
    return await $(n, { id: W(), targetId: t.id, shardId: r, path: i, sha256: "", size: 0, status: "failed", reason: p, at: c }), { status: "failed", reason: p };
  }
...
function J(s, t) {
  ...
  return {
    ...
    async putObject(i) {
      const l = {};
      for (const [m, k] of Object.entries(i.metadata ?? {}))
        l[`x-amz-meta-${m}`] = k;
      return { etag: (await r("PUT", `/${i.key}`, {
        body: i.body,
        headers: {
    ...
    async headObject(i) {
      try {
        const l = await r("HEAD", `/${i}`);
        return {
          size: Number(l.headers.get("content-length") ?? 0),
          sha256: l.headers.get("x-amz-meta-sh3-sha256") ?? void 0,
          etag: l.headers.get("etag") ?? "",
          lastModified: l.headers.get("last-modified") ?? void 0
        };
      } catch (l) {
        if (l instanceof Error && "status" in l && l.status === 404)
          return null;
        throw l;
      }
    },
    listObjectsV2({ prefix: i }) {
      return {
        async *[Symbol.asyncIterator]() {
          var c, m, k, y, p;
          let l = null;
          for (; ; ) {
            const A = new URLSearchParams({ "list-type": "2", prefix: i });
            l && A.set("continuation-token", l);
            const S = await (await r("GET", "/", { query: A.toString() })).text();
            for (const R of S.matchAll(/<Contents>([\s\S]*?)<\/Contents>/g)) {
              const g = R[1], u = ((c = /<Key>([^<]*)<\/Key>/.exec(g)) == null ? void 0 : c[1]) ?? "", _ = Number(((m = /<Size>([^<]*)<\/Size>/.exec(g)) == null ? void 0 : m[1]) ?? "0"), b = ((k = /<LastModified>([^<]*)<\/LastModified>/.exec(g)) == null ? void 0 : k[1]) ?? "";
              yield { key: u, size: _, lastModified: b };
            }
            if (l = ((y = /<IsTruncated>([^<]*)<\/IsTruncated>/.exec(S)) == null ? void 0 : y[1]) === "true" ? ((p = /<NextContinuationToken>([^<]*)<\/NextContinuationToken>/.exec(S)) == null ? void 0 : p[1]) ?? null : null, !l) break;
          }
        }
      };
    },
    async getObject(i) {
      return (await r("GET", `/${i}`)).text();
    }
  };
}
...
function re(s, t) {
  ...
  async function k(w) {
    ...
    const j = {
      ...
      bucket: e.get(o).trim(),
      region: "auto",
      keyPrefix: e.get(r).trim(),
      accessKeyId: e.get(i).trim(),
      secretAccessKey: e.get(l).trim()
    };
    try {
      await J(j).headBucket(), await t.onSave(j);
    } catch (F) {
      e.set(m, F instanceof Error ? F.message : String(F), !0);
    } finally {
      e.set(c, !1);
    }
  }
  var y = je(), p = e.child(y), A = e.sibling(e.child(p));
  e.remove_input_defaults(A), e.reset(p);
  var D = e.sibling(p, 2), S = e.sibling(e.child(D));
  e.remove_input_defaults(S), e.reset(D);
  var O = e.sibling(D, 2), R = e.sibling(e.child(O));
  e.remove_input_defaults(R), e.reset(O);
  var g = e.sibling(O, 2), u = e.sibling(e.child(g));
  e.remove_input_defaults(u), e.reset(g);
  var _ = e.sibling(g, 2), b = e.sibling(e.child(_));
  e.remove_input_defaults(b), e.reset(_);
  var f = e.sibling(_, 2), d = e.sibling(e.child(f));
  e.remove_input_defaults(d), e.reset(f);
  var I = e.sibling(f, 2);
  {
    var H = (w) => {
      var j = Ee(), z = e.child(j, !0);
      e.reset(j), e.template_effect(() => e.set_text(z, e.get(m))), e.append(w, j);
    };
    e.if(I, (w) => {
      e.get(m) && w(H);
    });
  ...
  var K = e.sibling(I, 2);
  {
    var L = (w) => {
      var j = Oe();
      e.delegated("click", j, function(...z) {
        var N;
        (N = t.onShowGuide) == null || N.apply(this, z);
      }), e.append(w, j);
    };
    e.if(K, (w) => {
      t.onShowGuide && w(L);
...
var He = e.from_html(`<aside class="r2-guide svelte-122r6hr" aria-label="Cloudflare R2 setup guide"><header class="r2-guide__header svelte-122r6hr"><h3 class="svelte-122r6hr">Cloudflare R2 setup</h3> <button class="r2-guide__close svelte-122r6hr" type="button" aria-label="Close guide">×</button></header> <ol class="r2-guide__steps svelte-122r6hr"><li class="svelte-122r6hr"><strong>Sign in to Cloudflare.</strong> Go to <a href="https://dash.cloudflare.com" target="_blank" rel="noopener" class="svelte-122r6hr">dash.cloudflare.com</a>.
      R2 requires a payment method to enable, but the free tier covers 10 GB of storage and 1M Class A / 10M Class B ops per month.</li> <li class="svelte-122r6hr"><strong>Enable R2 and create a bucket.</strong> From the left sidebar, click <em>R2</em>. If R2 isn't enabled, click <em>Enable R2</em> and accept the terms.
...
function De(s, t) {
  ...
  var o = He(), r = e.child(o), i = e.sibling(e.child(r), 2);
  e.reset(r);
  var l = e.sibling(r, 2), c = e.sibling(e.child(l), 8), m = e.sibling(e.child(c), 8), k = e.child(m, !0);
...
function Ne(s, t) {
  ...
  var c = ze();
  let m;
  var k = e.child(c), y = e.child(k), p = e.sibling(e.child(y), 2), A = e.child(p);
  {
    var D = (d) => {
      var I = Ke();
      e.delegated("click", I, i), e.append(d, I);
    };
    e.if(A, (d) => {
      e.get(n) || d(D);
    });
    ...
    var g = (d) => {
      ...
      var I = e.comment(), H = e.first_child(I);
      {
        var K = (h) => {
          ...
          var v = Be(), E = e.sibling(e.child(v));
          e.each(E, 21, () => t.rt.targets, (C) => C.id, (C, T) => {
            var P = Ue(), U = e.child(P), B = e.child(U, !0);
            e.reset(U);
            var q = e.sibling(U), w = e.child(q, !0);
            e.reset(q);
            var j = e.sibling(q), z = e.child(j, !0);
            e.reset(j);
        ...
        e.if(H, (h) => {
          t.rt.targetsLoaded ? t.rt.targets.length === 0 ? h(L, 1) : h(x, -1) : h(K);
        });
      }
      e.append(d, I);
    };
  ...
  {
    var f = (d) => {
      var I = qe(), H = e.child(I);
      De(H, { onClose: l }), e.reset(I), e.append(d, I);
    };
    e.if(b, (d) => {
      e.get(n) && d(f);
    });
...
function $e(s, t) {
  ...
  {
    var r = (i) => {
      var l = We(), c = e.child(l), m = e.child(c);
      {
        var k = (u) => {
          var _ = e.text();
          e.template_effect(() => e.set_text(_, `Running: ${t.p.currentLabel ?? ""}`)), e.append(u, _);
        }, y = (u) => {
          var _ = e.text("Done.");
          e.append(u, _);
        };
        e.if(m, (u) => {
          t.p.running ? u(k) : u(y, -1);
        });
      }
      e.reset(c);
      var p = e.sibling(c, 2), A = e.child(p);
      let D;
      e.reset(p);
      var S = e.sibling(p, 2), O = e.child(S);
      e.reset(S);
      var R = e.sibling(S, 2);
      {
        var g = (u) => {
          var _ = Ve(), b = e.child(_), f = e.child(b);
          e.reset(b);
          var d = e.sibling(b, 2);
          e.each(d, 21, () => t.p.errors, e.index, (I, H) => {
            var K = Ge(), L = e.child(K, !0);
            e.reset(K), e.template_effect(() => e.set_text(L, e.get(H))), e.append(I, K);
          }), e.reset(d), e.reset(_), e.template_effect(() => e.set_text(f, `${t.p.errors.length ?? ""} error${t.p.errors.length === 1 ? "" : "s"}`)), e.append(u, _);
        };
        e.if(R, (u) => {
          t.p.errors.length > 0 && u(g);
        });
      }
  ...
  var y = at(), p = e.sibling(e.child(y), 2);
  {
    var A = (S) => {
      ...
      e.each(g, 21, () => a().targets, (h) => h.id, (h, v) => {
        ...
        var T = {};
        e.template_effect(() => {
          e.set_text(C, `${e.get(v).label ?? ""} (${e.get(v).bucket ?? ""})`), T !== (T = e.get(v).id) && (E.value = (E.__value = e.get(v).id) ?? "");
        }), e.append(h, E);
      }), e.reset(g), e.reset(R);
      var u = e.sibling(R, 2), _ = e.sibling(e.child(u));
      e.each(_, 21, () => e.get(i), e.index, (h, v) => {
        var E = Xe(), C = e.child(E, !0);
        e.reset(E);
        var T = {};
        e.template_effect(() => {
          e.set_text(C, e.get(v)), T !== (T = e.get(v)) && (E.value = (E.__value = e.get(v)) ?? "");
        }), e.append(h, E);
      }), e.reset(_), e.reset(u);
      var b = e.sibling(u, 2), f = e.sibling(e.child(b));
      e.remove_input_defaults(f), e.reset(b);
      var d = e.sibling(b, 2), I = e.child(d, !0);
      e.reset(d);
      var H = e.sibling(d, 2);
      $e(H, {
        get p() {
      ...
      var K = e.sibling(H, 4);
      {
        var L = (h) => {
          ...
          var v = tt();
          e.each(v, 21, () => e.get(l), e.index, (E, C) => {
            var T = et(), P = e.child(T), U = e.child(P, !0);
            e.reset(P);
            var B = e.sibling(P, 2), q = e.child(B);
            e.reset(B);
            var w = e.sibling(B, 2), j = e.child(w, !0);
            e.reset(w);
            var z = e.sibling(w, 2);
            {
              var N = (F) => {
                var G = Qe(), ie = e.child(G);
                e.reset(G), e.template_effect(() => e.set_text(ie, `— ${e.get(C).reason ?? ""}`)), e.append(F, G);
              };
              e.if(z, (F) => {
                e.get(C).reason && F(N);
              });
            ...
            e.reset(T), e.template_effect(() => {
              e.set_class(T, 1, `status-${e.get(C).status}`, "svelte-8kfsmf"), e.set_text(U, e.get(C).at), e.set_text(q, `${e.get(C).shardId ?? ""}/${e.get(C).path ?? ""}`), e.set_text(j, e.get(C).status);
            }), e.append(E, T);
          }), e.reset(v), e.append(h, v);
        };
        e.if(K, (h) => {
          e.get(l).length === 0 ? h(L) : h(x, -1);
        });
      }
      e.template_effect(() => {
...
function lt(s, t) {
  ...
  e.each(n, 21, () => t.nodes, (o) => o.path, (o, r) => {
    ...
    var m = e.sibling(l), k = e.child(m), y = e.sibling(k);
    {
      var p = (R) => {
        var g = nt();
        e.append(R, g);
      ...
      e.if(y, (R) => {
        e.get(r).existsLocal && R(p);
      });
    }
    e.reset(m);
    var A = e.sibling(m), D = e.child(A, !0);
    e.reset(A);
    var S = e.sibling(A), O = e.child(S, !0);
    e.reset(S), e.reset(i), e.template_effect(
      (R) => {
        e.set_class(i, 1, e.clsx(e.get(r).existsLocal ? "exists" : ""), "svelte-8cyv1p"), e.set_checked(c, R), e.set_text(k, `${e.get(r).path ?? ""} `), e.set_text(D, e.get(r).size), e.set_text(O, e.get(r).lastModified);
      },
...
function bt(s, t) {
  ...
  async function m() {
    ...
    if (!(!g || !t.rt.ctx.browse)) {
      ...
      try {
        const u = J(g), _ = /* @__PURE__ */ new Set();
        for (const d of await t.rt.ctx.browse.listDocuments()) _.add(`${d.shardId}/${d.path}`);
        const b = [], f = /* @__PURE__ */ new Set();
        for await (const d of u.listObjectsV2({ prefix: g.keyPrefix })) {
          const I = d.key.slice(g.keyPrefix.length), H = I.indexOf("/");
          if (H < 0) continue;
          const K = I.slice(0, H), L = I.slice(H + 1), x = _.has(`${K}/${L}`);
          b.push({
            path: d.key,
            size: d.size,
  ...
  async function y() {
    ...
    try {
      const _ = J(g), b = Re(t.rt.ctx);
      for (const f of e.get(o)) {
        const d = e.get(n).find((x) => x.path === f);
        if (!d) continue;
        const I = f.slice(g.keyPrefix.length), H = I.indexOf("/"), K = I.slice(0, H), L = I.slice(H + 1);
        if (d.existsLocal) {
          u.skipped++;
          continue;
        }
        try {
          ...
          u.failed++, u.errors.push(`${f}: ${x instanceof Error ? x.message : String(x)}`);
        }
      }
    } finally {
      e.set(i, !1), e.set(c, u, !0);
    }
  }
  var p = mt(), A = e.sibling(e.child(p), 2);
  {
    var D = (g) => {
      var u = ct();
      e.append(g, u);
  ...
  var S = e.sibling(A, 2);
  {
    var O = (g) => {
      ...
      var u = vt(), _ = e.first_child(u), b = e.sibling(e.child(_));
      e.each(b, 21, () => t.rt.targets, (x) => x.id, (x, h) => {
        var v = ut(), E = e.child(v);
        e.reset(v);
        var C = {};
        e.template_effect(() => {
          e.set_text(E, `${e.get(h).label ?? ""} (${e.get(h).bucket ?? ""})`), C !== (C = e.get(h).id) && (v.value = (v.__value = e.get(h).id) ?? "");
        }), e.append(x, v);
      }), e.reset(b), e.reset(_);
      var f = e.sibling(_, 2), d = e.child(f, !0);
      e.reset(f);
      var I = e.sibling(f, 2);
      {
        var H = (x) => {
          var h = gt(), v = e.first_child(h), E = e.child(v);
          e.reset(v);
          var C = e.sibling(v, 2);
          lt(C, {
            get nodes() {
          ...
          var T = e.sibling(C, 2), P = e.child(T, !0);
          e.reset(T), e.template_effect(() => {
            e.set_text(E, `${e.get(n).length ?? ""} object${e.get(n).length === 1 ? "" : "s"} — ${e.get(o).size ?? ""} selected`), T.disabled = e.get(i) || e.get(o).size === 0, e.set_text(P, e.get(i) ? "Importing…" : `Import ${e.get(o).size} object${e.get(o).size === 1 ? "" : "s"}`);
          }), e.delegated("click", T, y), e.append(x, h);
        };
        e.if(I, (x) => {
          e.get(n).length > 0 && x(H);
        });
      }
      var K = e.sibling(I, 2);
      {
        var L = (x) => {
          var h = ft(), v = e.child(h), E = e.sibling(v);
          {
            var C = (T) => {
              var P = pt(), U = e.sibling(e.child(P));
              e.each(U, 21, () => e.get(c).errors, e.index, (B, q) => {
                var w = ht(), j = e.child(w, !0);
                e.reset(w), e.template_effect(() => e.set_text(j, e.get(q))), e.append(B, w);
              }), e.reset(U), e.reset(P), e.append(T, P);
            };
            e.if(E, (T) => {
              e.get(c).errors.length > 0 && T(C);
            });
          }
          e.reset(h), e.template_effect(() => e.set_text(v, `Imported: ${e.get(c).imported ?? ""} · Skipped (exists locally): ${e.get(c).skipped ?? ""} · Failed: ${e.get(c).failed ?? ""} `)), e.append(x, h);
        };
        e.if(K, (x) => {
          e.get(c) && x(L);
        });
