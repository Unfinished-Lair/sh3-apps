<script lang="ts">
  ...
  interface Props {
    ...
    compact?: boolean;
    /** Called when the user saves a new user palette (shard appends to ctx.state). */
    onSaveUserPalette?: (palette: ColorPalette) => void;
    /** Called when the user deletes a user palette (shard splices from ctx.state). */
  ...
  let mode = $state<'hsv' | 'rgb'>(prefs.mode);

  // Sync inbound entry.value changes (e.g., undo/redo writing to registry) into local hsv.
  $effect(() => {
    const current = entry ? entry.value : (normalizeHex(adHocValue ?? '') ?? '#000000');
  ...
  function pushValue(nextHex: string) {
    ...
    if (onExternalCommit) {
      // Inspector-embedded path: let the parent walker own history + parent write.
      onExternalCommit(normalized);
      lastSyncedHex = normalized;
      hsv = hexToHsv(normalized);
      return;
    }
    const prev = entry ? entry.value : (normalizeHex(adHocValue ?? '') ?? '#000000');
  ...
  // Re-sync after history events (undo / redo writes registry → hsv).
  $effect(() => {
  ...
  // Redraw on any hsv change. The square re-renders only when h changes;
  // otherwise the existing image data is replaced and the indicator redrawn.
  ...
  function onPopoverBackdrop(e: MouseEvent) {
    ...
    if (target && target.closest('.cp-popover-content, .cp-compact-trigger')) return;
    popoverOpen = false;
  }

  $effect(() => {
    if (!popoverOpen) return;
    document.addEventListener('mousedown', onPopoverBackdrop, true);
    return () => document.removeEventListener('mousedown', onPopoverBackdrop, true);
  });
...
{#if compact}
  ...
  >
    <div class="cp-compact-row">
      <div class="cp-preview cp-compact-preview" style:background-color={currentHex}></div>
      <input
        type="text"
    ...
    {#if popoverOpen}
      <div class="cp-popover-content">
        {@render squareAndStrip()}
        {@render modeToggle()}
        {@render sliderBlock()}
        {@render paletteColumn()}
      </div>
    {/if}
  </div>
{:else}
...
<style>
  ...
  .cp-mode button {
    ...
    font-size: 11px;
    padding: 3px 10px;
    background: var(--shell-bg-lighter);
  ...
  .cp-mode button.active {
    background: var(--shell-accent, #3a7eff);
    color: var(--shell-bg);
  }
  ...
  .cp-range {
    ...
    height: 8px;
    border-radius: 4px;
    background: var(--track-bg, var(--shell-bg-lighter));
    outline: none;
    cursor: pointer;
  }
  .cp-range::-webkit-slider-thumb {
    ...
    height: 14px;
    border-radius: 50%;
    background: var(--shell-text-bright);
    border: 2px solid var(--shell-border-light);
    cursor: pointer;
  }
  ...
  .cp-compact-preview { width: 20px; height: 20px; }
  .cp-compact-hex { width: 90px; font-size: 12px; }
  .cp-compact-trigger {
    appearance: none;
    font: inherit;
    font-size: 14px;
  ...
  .cp-compact-trigger[disabled] { opacity: 0.5; cursor: not-allowed; }
